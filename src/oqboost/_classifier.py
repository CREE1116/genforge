...
class OQBoostClassifier(BaseEstimator, ClassifierMixin, TransformerMixin):
    ...
    inherited_rp_ratio : float
        Fraction of oblique-direction candidates drawn from the informed
        pool (cache-blend: directions that scored well in earlier trees,
        blended with the parent direction) vs fresh sparse random
        projections. The default 0.5 splits the budget evenly — validated
        as never-worse than other mixes across categorical, financial and
        synthetic benchmarks (research/FINDINGS.md Part 2).
    mutation_rate : float
        Deprecated, ignored. Belonged to the removed parent-mutation
        Strategy A (ablation showed parent-direction mutation underperforms
        random replacement). Accepted for backwards compatibility.
    mutation_strength : float
        Deprecated, ignored (removed Strategy B). Accepted for backwards
        compatibility.
    """
