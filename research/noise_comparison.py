"""
Compare child direction mutation strategies with different noise scaling:
1. uniform: Constant noise scale (0.5) at all depths
2. less: Decreasing noise scale as depth increases (max(0.1, 1.0 - 0.3 * depth))
3. more: Increasing noise scale as depth increases (min(1.5, 0.1 + 0.3 * depth))
"""
...
def main():
    # 1. Rotated Synthetic (4000 samples total, to keep it fast)
