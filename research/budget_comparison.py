"""
Compare three budget allocation strategies for random child directions:
1. uniform: 8 candidates per node at all depths.
2. less: Fewer candidates as depth increases (depth 1 -> 14, depth 2 -> 8, depth 3 -> 2).
3. more: More candidates as depth increases (depth 1 -> 2, depth 2 -> 8, depth 3 -> 14).
"""
...
def run_experiment(X_tr, y_tr, X_te, y_te, dataset_name, n_seeds=5):
    ...
    for strategy in strategies:
        ...
        print(f"Running '{strategy}' budget strategy...", end="", flush=True)
        t0 = time.time()
        for seed in range(n_seeds):
            clf = OQBoostResearch(
                ...
                use_wls=True,
                inherit_mode='orth',
                orth_strategy='random',  # Completely random direction
...
def main():
    ...
    print("\n\n### Summary of Budget Allocation Results (Mean ± Std over 3 seeds)\n")
    print("| Dataset | Budget Strategy | Test Accuracy ↑ | Test Log Loss ↓ |")
    print("|---|---|---|---|")
    for dataset, res in all_results.items():
        for strategy in ["uniform", "less", "more"]:
