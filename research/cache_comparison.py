"""
Compare cache strategy enhancements:
1. cache_blend (Baseline): C++ style FIFO cache with standard parent blend
2. cache_orb: FIFO cache with Orthogonal Random Blending (ORB)
3. cache_evolutionary: Evolutionary Cache Replacement (ECR) with ORB
"""
...
def run_experiment(X_tr, y_tr, X_te, y_te, dataset_name, n_seeds=3):
    ...
    configs = [
        ...
        {"name": "cache_orb", "inherit_mode": "cache_orb"},
        {"name": "cache_evolutionary", "inherit_mode": "cache_evolutionary"},
    ]
    results = {}
...
def main():
    ...
    for dataset, res in all_results.items():
        for name in ["cache_blend", "cache_orb", "cache_evolutionary"]:
