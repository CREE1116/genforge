"""Deployment-protocol validation of the depth-adaptive candidate budget.

THEORY.md §2/P4-refined: deep nodes are noise-dominated, so a uniform
per-node budget overspends there. Gated by OQB_BUDGET_MODE:
  0 = uniform 32-candidate pool everywhere (production baseline)
  1 = cut only: 32 at depth <= 2, 8 at depth >= 3 (isolates the speed win)
  2 = cut + reinvest: 64 at depth <= 2, 8 at depth >= 3
...
Usage:
  python research/budget_experiment.py            # orchestrate all runs
  python research/budget_experiment.py WORKER <dataset> <mode>   # internal
"""
...
def orchestrate() -> None:
    out_path = ROOT / "research" / "budget_experiment_results.json"
    results = []
    for dataset in DATASETS:
        for mode in MODES:
            env = dict(os.environ, OQB_BUDGET_MODE=str(mode))
            t0 = time.perf_counter()
            p = subprocess.run(
