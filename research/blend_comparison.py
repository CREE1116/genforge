"""
Compare adaptive cache blend ratio strategies:
1. cache_blend (Baseline): Uniform random blend weight alpha ~ U(0.2, 0.8)
2. cache_adaptive_blend_size: Sample ratio adaptive weight alpha
3. cache_adaptive_blend_gradient: Gradient variance adaptive weight alpha
4. cache_adaptive_blend_depth_prop: Depth proportional weight alpha (grows with depth)
5. cache_adaptive_blend_depth_inv: Depth inverse weight alpha (decays with depth)
"""
...
def run_experiment(X_tr, y_tr, X_te, y_te, dataset_name, n_seeds=5):
    print(f"\n==================================================")
    print(f"Dataset: {dataset_name} ({X_tr.shape[0]} train, {X_te.shape[0]} test, {X_tr.shape[1]} features)")
    print(f"==================================================")
    
    configs = [
        {"name": "cache_blend", "inherit_mode": "cache_blend"},
        {"name": "cache_adaptive_blend_size", "inherit_mode": "cache_adaptive_blend_size"},
        {"name": "cache_adaptive_blend_gradient", "inherit_mode": "cache_adaptive_blend_gradient"},
        {"name": "cache_adaptive_blend_depth_prop", "inherit_mode": "cache_adaptive_blend_depth_prop"},
        {"name": "cache_adaptive_blend_depth_inv", "inherit_mode": "cache_adaptive_blend_depth_inv"},
    ]
    results = {}
...
def main():
    ...
    all_results = {}
    all_results["rotated_synthetic"] = run_experiment(X_tr_rot, y_tr_rot, X_te_rot, y_te_rot, "Rotated Synthetic (binary)", n_seeds=5)
    all_results["digits"] = run_experiment(X_tr_dg, y_tr_dg, X_te_dg, y_te_dg, "Digits (multiclass, 10 classes)", n_seeds=5)
    all_results["wine"] = run_experiment(X_tr_wn, y_tr_wn, X_te_wn, y_te_wn, "Wine (multiclass, 3 classes)", n_seeds=5)
    all_results["synthetic"] = run_experiment(X_tr_syn, y_tr_syn, X_te_syn, y_te_syn, "Synthetic Correlated", n_seeds=5)
    ...
    for dataset, res in all_results.items():
        for name in ["cache_blend", "cache_adaptive_blend_size", "cache_adaptive_blend_gradient", "cache_adaptive_blend_depth_prop", "cache_adaptive_blend_depth_inv"]:
