"""
Compare three child direction orthogonality strategies:
1. full: Complete orthogonality (beta = 1.0)
2. more: More orthogonal as depth increases (beta = depth / (depth + 1.0))
3. less: Less orthogonal as depth increases (beta = 1.0 / (depth + 1.0))
"""
...
def run_experiment(X_tr, y_tr, X_te, y_te, dataset_name, n_seeds=5):
    ...
    strategies = ["full", "more", "less", "random"]
    results = {}
    ...
    for strategy in strategies:
        ...
        print(f" done. Acc: {mean_acc:.4f}±{std_acc:.4f} | Loss: {mean_loss:.4f}±{std_loss:.4f} ({elapsed:.1f}s total)")
        
    return results


def main():
    # 1. Rotated Synthetic (Oblique decision boundary testbed)
    # We generate 10,000 base samples and apply a random rotation.
    ...
    # 4. Synthetic correlated (E7 setting)
    X_syn, y_syn = make_classification(
        n_samples=4000, n_features=30, n_informative=12, n_redundant=10,
    ...
    print("| Dataset | Strategy | Test Accuracy ↑ | Test Log Loss ↓ |")
    print("|---|---|---|---|")
    for dataset, res in all_results.items():
        for strategy in ["full", "more", "less", "random"]:
            acc_str = f"{res[strategy]['acc_mean']:.4f} ± {res[strategy]['acc_std']:.4f}"
            loss_str = f"{res[strategy]['loss_mean']:.4f} ± {res[strategy]['loss_std']:.4f}"
            print(f"| {dataset} | **{strategy}** | {acc_str} | {loss_str} |")
